...
import {
  User, Building2, Globe, Phone, Linkedin, Mail,
  Settings, Palette, PenLine, Zap, Server, Check,
...
export default function SettingsPage() {
  ...
  const saveSmtp = async () => {
    setSavingSmtp(true)
    try {
      await profileApi.updateSmtp(smtp)
      await refreshProfile()
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="mb-2">
        ...
        <p className="page-sub">Your profile context is injected into every AI-generated email and WhatsApp message</p>
      </div>
      ...
      <Section title="Your Company" icon={Building2}>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <Field label="Company tagline" hint="One-line description used in AI prompts">
      ...
      <Section title="Outreach Preferences" icon={PenLine}>
        <Field label="Preferred tone" hint="Affects how Groq writes your emails and WA messages">
          <div className="flex gap-2 flex-wrap mt-1">
            {TONES.map(t => (
              <button
                key={t}
                onClick={() => set('preferred_tone', t)}
        ...
        <Field label="Intro line" hint="How you like to open cold emails (used by AI)">
        ...
        <Field label="Value proposition" hint="What you offer — AI weaves this into every message">
          <textarea className="textarea h-24" value={form.value_proposition || ''} onChange={e => set('value_proposition', e.target.value)}
            placeholder="We help [industry] businesses increase revenue by 30% through automated lead outreach…" />
        </Field>

        <Field label="Email signature HTML" hint="Appended to every outbound email">
          <textarea className="textarea h-28 font-mono text-xs" value={form.email_signature_html || ''} onChange={e => set('email_signature_html', e.target.value)}
            placeholder={'<p><strong>John Smith</strong></p>\n<p>Business Development · Acme Corp</p>'} />
        </Field>
      </Section>
      ...
      {/* SMTP */}
      <Section title="SMTP / Email Settings" icon={Server}>
        <p className="text-xs text-slate-500 -mt-2 mb-2">
          These are read from your <code className="mono bg-slate-100 px-1.5 py-0.5 rounded text-xs">.env</code> file on the server.
          Update here to override profile-level SMTP (env vars always take priority).
        </p>
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="TLS">
            <div className="flex items-center gap-3 mt-2">
              <button onClick={() => setSmtp(p => ({...p, use_tls: !p.use_tls}))}
                className={`w-11 h-6 rounded-full transition-all relative ${smtp.use_tls ? 'bg-blue-500' : 'bg-slate-200'}`}>
                <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${smtp.use_tls ? 'left-5' : 'left-0.5'}`} />
              </button>
              <span className="text-sm text-slate-600">{smtp.use_tls ? 'STARTTLS enabled' : 'TLS disabled'}</span>
            </div>
          </Field>
        </div>
        <div className="flex gap-3 pt-2">
          <button onClick={saveSmtp} disabled={savingSmtp} className="btn-primary">
            {savingSmtp ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />}
            {savingSmtp ? 'Saving…' : 'Save SMTP'}
          </button>
          <button onClick={testSmtp} disabled={testing} className="btn-secondary">
            {testing ? <Loader2 size={15} className="animate-spin" /> : <Server size={15} />}
